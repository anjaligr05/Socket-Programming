...
int main() {
	...
	inet_ntop(p->ai_family, get_in_addr((struct sockaddr *) p->ai_addr), s,
			sizeof s);
	//printf("ServerB: connecting to %s\n", s);
	printf("The ServerB is up and running.\n");

	printf("The Server B has UDP port number %s and IP address %s\n",
	SERVER_B_PORT, s);
	...
	strcat(message_buffer, "End of file :B");
	...
	if ((rv_udp = getaddrinfo(NULL, SERVER_B_PORT, &hints_udp, &servinfo_udp))
			!= 0) {
		fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv_udp));
		return 1;
	}
	...
	/*struct sockaddr_in udp_addr;
			...
			getpeername(sockfd_udp,(struct sockaddr*)&udp_addr,&udp_addr_len);*/



	printf("\nThe server B has received the network topology from the Client with UDP port number %d and IP address %s\n(Client’s UDP port number and IP address) as follows:\n",ntohs(their_addr_udp.sin_port),s);
	printf("Edge------Cost\n");

	/*printf("SeverB: got packet from %s\n",
	...
	printf(
			"For this connection with Client, The Server B has UDP port number %s and IP address %s.\n",
			SERVER_B_PORT, s);
	close(sockfd_udp);
