...
int main() {
	...
	printf("The ServerA is up and running.\n");
	...
	printf("The Server A has UDP port number %s and IP address %s\n",
				SERVER_A_PORT, s);
	...
	strcat(message_buffer, "End of file :A");
	...
	if ((rv_udp = getaddrinfo(NULL, SERVER_A_PORT, &hints_udp, &servinfo_udp))
			!= 0) {
		fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv_udp));
		return 1;
	}
	...
	if ((numbytes_udp = recvfrom(sockfd_udp, buf_udp, MAXLENGTH - 1, 0,
		...
		printf("Edge------Cost\n");
	/*printf("SeverA: got packet from %s\n",
