...
int main() {
	...
	//printf("ServerD: connecting to %s\n", s);

	printf("The Server D has UDP port number %s and IP address %s\n",
					SERVER_D_PORT, localIP);
	...
	strcat(message_buffer, "End of file :D");
	if ((numbytes = send(sockfd, message_buffer, strlen(message_buffer), 0))
			== -1) {
		perror("send");
		exit(1);
	}
	...
	if ((rv_udp = getaddrinfo(NULL, SERVER_D_PORT, &hints_udp, &servinfo_udp))
			!= 0) {
		fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv_udp));
		return 1;
	}
	...
	if (p_udp == NULL) {
		fprintf(stderr, "ServerD: failed to bind socket\n");
		return 2;
	}

	freeaddrinfo(servinfo_udp);

	//printf("ServerD: waiting to recvfrom...\n");
	...
	/*printf("SeverD: got packet from %s\n",
			inet_ntop(their_addr_udp.ss_family,
					get_in_addr((struct sockaddr *) &their_addr_udp), s_udp,
					sizeof s_udp));*/
	//printf("ServerD: packet is %d bytes long\n", numbytes_udp);
	buf_udp[numbytes_udp] = '\0';
	//printf("ServerD: packet contains \"%s\"\n", buf_udp);
	printf("%s\n",buf_udp);
	printf("For this connection with Client, The Server D has UDP port number %s and IP address %s.\n",SERVER_D_PORT,s);
	close(sockfd_udp);
